...
const ROUTES = [
  // ─── Pipeline Action Routes (Phase 3) ─────────────────────────────────────
  {
    name: 'claim_lead',
    match: (p) => p.eventType === 'claim_lead',
    handler: async (p) => getHandlers().stage.claimLead(p),
    skipDuplicate: true,
  },

  {
    name: 'transition_stage',
    match: (p) => p.eventType === 'transition_stage',
    handler: async (p) => getHandlers().stage.transitionStage(p),
    skipDuplicate: true,
  },

  {
    name: 'confirm_payment',
    match: (p) => p.eventType === 'confirm_payment',
    handler: async (p) => getHandlers().stage.confirmPayment(p),
    skipDuplicate: true,
  },

  {
    name: 'get_lead_history',
    match: (p) => p.eventType === 'get_lead_history',
    handler: async (p) => getHandlers().stage.getLeadHistory(p),
    skipDuplicate: true,
  },

  // ─── Outbound Sync (Phase 3) ──────────────────────────────────────────────
  {
    name: 'outbound_sync',
    match: (p) => p.eventType === 'outbound_sync',
    handler: async (p) => getServices().syncService.syncToSheets(),
    skipDuplicate: true,
  },
