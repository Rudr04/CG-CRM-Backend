...
function getHandlers() {
  if (!_handlers) {
    _handlers = {
      ...
      stage: require('../handlers/stageHandler'),
    };
  }
  return _handlers;
}
...
const ROUTES = [
  // ─── Stage Transition (from GAS Stage column edit) ────────────────────────
  {
    name: 'stage_transition',
    match: (p) => p.eventType === config.EVENT_TYPES.STAGE_TRANSITION,
    handler: async (p) => getHandlers().stage.handleStageTransition(p),
    skipDuplicate: true,
  },
  ...
  {
    name: 'web_form',
    match: (p) => p.event_type === config.EVENT_TYPES.CG_WEB,
  ...
  },
  {
    ...
    match: (p) => p.eventType === config.EVENT_TYPES.MESSAGE &&
                  p.text &&
                  p.text.trim() === 'Join Free Demo Class',
    handler: async (p) => getHandlers().contact.handleReactivation(p),
  },

  // ─── Scholarship Claim Reply ──────────────────────────────────────────────
  // {
  //   name: 'scholarship_claim',
  //   match: (p) => p.eventType === config.EVENT_TYPES.MESSAGE &&
  //                 p.text &&
  //                 (p.text.trim() === 'હા સ્કોલરશીપ ક્લેમ કરો' ||
  //                  p.text.trim() === 'हाँ, स्कॉलरशिप क्लेम करें'),
  //   handler: async (p) => getHandlers().contact.handleScholarshipClaim(p),
  // },
  ...
  // ─── CVPT keyword (Premium + Classic Pass) ────────────────────────────────
  {
    name: 'cvpt_keyword',
    match: (p) => p.eventType === config.EVENT_TYPES.MESSAGE &&
                  p.text && (
                    config.CVPT.KEYWORDS.PREMIUM.includes(p.text.trim()) ||
                    config.CVPT.KEYWORDS.CLASSIC.includes(p.text.trim())
                  ),
    handler: async (p) => getHandlers().contact.handleCVPTKeyword(p),
  },
  ...
  // ─── Scheduled Follow-up (from Cloud Tasks) ─────────────────────────────
  {
    name: 'scheduled_followup',
    match: (p) => p.eventType === config.EVENT_TYPES.SCHEDULED_FOLLOWUP,
    handler: async (p) => getHandlers().contact.handleScheduledFollowup(p),
    skipDuplicate: true,
  },

  // ─── Scheduled Inactive Check (from Cloud Tasks, 48hrs) ─────────────────
  {
    name: 'scheduled_inactive_check',
    match: (p) => p.eventType === 'scheduled_inactive_check',
    handler: async (p) => getHandlers().contact.handleInactiveCheck(p),
    skipDuplicate: true,
  },
  
  // ─── Razorpay Premium Pass Payment ────────────────────────────────────────
  {
    name: 'razorpay_premium_payment',
    match: (p) => p.event === 'payment_link.paid' &&
                  p.payload?.payment_link?.entity?.reference_id?.startsWith('PP-'),
    handler: async (p) => getHandlers().payment.handlePremiumPassPayment(p),
    skipDuplicate: true,
  },
...
async function routeEvent(params) {
  ...
  if (!match) {
    const eventType = params.eventType || params.event_type || params.type || params.event || 'unknown';
