...
const ROUTES = [
  ...
  // ─── Serious Learner Pass keyword ─────────────────────────────────────────
  {
    name: 'serious_learner_keyword',
    match: (p) => p.eventType === config.EVENT_TYPES.MESSAGE &&
                  p.text && config.SLP.KEYWORDS.includes(p.text.trim()),
    handler: async (p) => getHandlers().contact.handleSeriousLearnerKeyword(p),
  },
  ...
  // ─── Razorpay SLP Payment ─────────────────────────────────────────────────
  {
    name: 'razorpay_slp_payment',
    match: (p) => p.event === 'payment_link.paid' &&
                  p.payload?.payment_link?.entity?.reference_id?.startsWith('SLP-'),
    handler: async (p) => getHandlers().payment.handleSeriousLearnerPayment(p),
    skipDuplicate: true,
  },
