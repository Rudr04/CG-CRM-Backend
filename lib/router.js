...
const ROUTES = [
  ...
  // ─── TEMP DEBUG: Razorpay raw payload capture ─────────────────────────────
  // Remove this block once full payload is captured and real handler is built.
{
    name: 'razorpay_debug_capture',
    match: (p) => typeof p.event === 'string' && p.event.startsWith('payment_link.'),
    handler: async (p) => {
      const { captureRazorpayPayload } = require('../handlers/debugHandler');
      return captureRazorpayPayload(p);
    },
    skipDuplicate: true,
  },
